...
public class ExtendedKalmanFilterTest {
    public static Matrix<N5, N1> getDynamics(final Matrix<N5, N1> x, final Matrix<N2, N1> u) {
        ...
        final var C1 = -Math.pow(gr, 2) * motors.KtNMPerAmp /
    ...
    public void testInit() {
        ...
        var R = StateSpaceUtils.makeCostMatrix(Nat.N5(), new MatBuilder<>(Nat.N5(), Nat.N1()).fill(0.01, 0.01, 0.0001, 0.5, 0.5));
    ...
    public void testConvergence() {
        ...
        var trajectory = TrajectoryGenerator.generateTrajectory(
            waypoints, new TrajectoryConfig(8.8, 0.1)
        );
        ...
        var B = NumericalJacobian.numericalJacobianU(
            ...
            MatrixUtils.zeros(Nat.N5(), Nat.N1()),
            u
        );
        ...
        for (int i = 0; i < (totalTime / dtSeconds); i++) {
            ...
            u = new Matrix<>(StateSpaceUtils.householderQrDecompose(B.getStorage()).solve(
                rdot.minus(getDynamics(r, MatrixUtils.zeros(Nat.N2(), Nat.N1()))).getStorage()
            ));
